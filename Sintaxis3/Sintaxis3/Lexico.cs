    ...
    class Lexico : Token, IDisposable
    {
        ...
        public Lexico(string nombreArch)
        {
            ...
            if (File.Exists(nombreArch))
            {
                ...
                if(extesion == ".cpp")
                {
                    NombreAr = Path.GetFileName(nombreArch);

                    string log = Path.ChangeExtension(nombreArch, "log");
                    string ruta = Path.GetDirectoryName(nombreArch);
                    ...
                    bitacora.WriteLine("Directorio: " + ruta);
                    bitacora.WriteLine("Fecha: " + fecha.ToString("D") + ", Hora: " + fecha.ToString("t"));
                }
                else
                {
        ...
        public void Dispose()
        {
            Console.WriteLine("Finaliza compilacion de " + NombreAr);
            CerrarArchivos();
        }
        ...
        protected void NextToken()
        {
            ...
            if (estado == E)
            {
                ...
                if (clas == clasificaciones.numero)
                {
                    ...
                    throw new Exception("Error lexico en la linea {0}, en el caracter {1}, se espera un digito");
                }
                else if (clas == clasificaciones.cadena)
                {
                    if (anterior == 22)
                    {
                        ...
                        throw new Exception("Error lexico en la linea {0}, en el caracter {1}, se esperan comillas dobles");
                    }
                    else if (anterior == 24)
                    {
                        bitacora.WriteLine("\nError lexico en la linea {0}, en el caracter {1}, se esperan comillas simples", linea, caracter);
                        throw new Exception("Error lexico en la linea {0}, en el caracter {1}, se esperan comillas simples");
                    }
                }
                else
                {
                    bitacora.WriteLine("\nError lexico en la line {0}, en el caracter {1}: No se cerró el comentario de manera correcta");
                    throw new Exception("Error lexico en la line {0}, en el caracter {1}: No se cerró el comentario de manera correcta");
                }
            }
